 ...
 * PATCH 时 name 与 label 都设为目标资产名，避免 GitHub UI 显示错误 label。
 * @param {object} opts
...
function uploadAsset({ repo, tag, localPath, name }) {
  ...
  // 无论文件名是否等于目标资产名，都 PATCH 设置 label=name（gh release upload 不设 label 时默认为空，
  // 会导致 GitHub UI 显示空白 label）；仅当文件名 != 资产名时才需要一并 PATCH name。
  const baseName = path.basename(localPath)
  const assetId = getAssetId(repo, tag, baseName)
  const fields = [`label=${name}`]
  if (baseName !== name) fields.unshift(`name=${name}`)
  const args = fields.reduce((acc, f) => [...acc, '-f', f], [])
  gh(['api', '--method', 'PATCH', ...args, `repos/${repo}/releases/assets/${assetId}`], repo)
}
