  ...
  for (const resourceId of Object.keys(manifest.resources)) {
    ...
    for (const platformKey of Object.keys(def.assets)) {
      ...
      if (existing.includes(name) && !force) {
        // 同名资产已在远端：非 force 模式下幂等跳过。
        // ⚠️ 注意：不能用本地文件的 sha256/size 更新 index！
        // 原因：本地文件可能是新的重新构建产物（sha256 与远端实际文件不同），
        // 若用本地文件信息覆盖 index，会导致 consume 下载的是远端旧文件但校验的是本地新 sha256，必然失败。
        // 正确做法：幂等跳过时完全不更新 index，保留 index 中现有的远端文件信息。
        // 如需强制重新发布（文件已变化），请使用 --force 参数。
